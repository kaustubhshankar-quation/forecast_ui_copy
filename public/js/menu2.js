...
(function(){
	var a = document.getElementById('innav').getElementsByTagName("a");
	// var a = document.getElementById('innav');
	// if(a === null){
	// 	a = document.getElementById('ininnav');
	// }
...
document.getElementById('innav').querySelector('ul').insertAdjacentHTML("beforebegin", "<span id='menutoggle' class='test'><span>Menu</span></span>");
...
menutoggle.onclick = function(event) {
	...
	var el = document.querySelectorAll("#innav span.submenu, #innav ul.submenu");
...
forEach(document.querySelectorAll("#innav span.submenu"), function (index, value) {
...
function hideMenu() {
	var el = document.querySelectorAll("#menutoggle, #menutoggle + ul, #innav span.submenu, #innav ul.submenu");
	var i; for (i = 0; i < el.length; i++) {
		el[i].classList.remove(activeClass);
	}
}
...
document.addEventListener('click', function(e) {
	if ( menutoggle.offsetWidth > 0 && menutoggle.offsetHeight > 0 ) { // if the #menutoggle is visible
		...
		if (!event_element.closest("#innav")){
			//console.log(event_element.closest("#innav"));
